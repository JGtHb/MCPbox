    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface rounded-lg shadow">
        <div className="px-6 py-4 border-b border-hl-med">
          ...
          <p className="text-xs text-muted mt-0.5">
            Revoke to remove the host from the server's allowlist
          </p>
        </div>
        {networkHosts.length === 0 ? (
          <div className="px-6 py-8 text-center">
            ...
            <p className="text-subtle">No approved network access for this server</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-hl-med">
                {networkHosts.map((req) => (
                  <tr key={req.id}>
                    ...
                    <td className="px-6 py-3 text-sm text-subtle">{req.tool_name}</td>
                    <td className="px-6 py-3 text-sm text-subtle">
                      {req.reviewed_by ?? '—'}
                    </td>
                    <td className="px-6 py-3 text-right">
