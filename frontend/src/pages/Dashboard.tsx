      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            ...
            <p
              className={`text-2xl font-bold ${
                tunnelStatus?.status === 'connected' ? 'text-green-600 dark:text-green-400' : 'text-gray-400'
              }`}
            >
              {tunnelStatus?.status === 'connected' ? 'Online' : 'Offline'}
            </p>
            <p className="text-xs text-gray-500 dark:text-gray-400">
              {tunnelStatus?.url ? 'Connected' : 'Not configured'}
            </p>
          </div>
        </div>
                ...
                {dashboard.requests_over_time.slice(-24).map((point, i) => {
                  ...
                  return (
                    <div
                      ...
                      title={`${new Date(point.timestamp).toLocaleTimeString()}: ${point.value} requests`}
                    />
                  )
                })}
              </div>
            ...
            <div className="flex justify-between text-xs text-gray-400 mt-2">
              <span>
                {dashboard?.requests_over_time?.[0]
                  ? new Date(dashboard.requests_over_time[0].timestamp).toLocaleTimeString([], {
                      hour: '2-digit',
                      minute: '2-digit',
                    })
                  : '-'}
              ...
              <span>
                {dashboard?.requests_over_time?.length
                  ? new Date(
                      dashboard.requests_over_time[dashboard.requests_over_time.length - 1].timestamp
                ...
                {dashboard.errors_over_time.slice(-24).map((point, i) => {
                  ...
                  return (
                    <div
                      ...
                      title={`${new Date(point.timestamp).toLocaleTimeString()}: ${point.value} errors`}
                    />
                  )
                })}
              </div>
            ) : (
              <div className="h-48 flex items-center justify-center text-green-600 text-sm">
            ...
            <div className="flex justify-between text-xs text-gray-400 mt-2">
              <span>
                {dashboard?.errors_over_time?.[0]
                  ? new Date(dashboard.errors_over_time[0].timestamp).toLocaleTimeString([], {
                      hour: '2-digit',
                      minute: '2-digit',
                    })
                  : '-'}
              ...
              <span>
                {dashboard?.errors_over_time?.length
                  ? new Date(
                      dashboard.errors_over_time[dashboard.errors_over_time.length - 1].timestamp
                    ).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                  : '-'}
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-medium text-gray-900">Servers</h3>
              <Link to="/servers" className="text-xs text-blue-600 hover:text-blue-800">
                View all
              </Link>
            </div>
                ...
                {dashboard.servers.slice(0, 5).map((server) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-1 text-xs text-gray-400">
                      ...
                      {server.errors_24h > 0 && (
                        <span className="text-red-500">{server.errors_24h} errors</span>
                      )}
                    </div>
                  </Link>
                ))}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link to="/activity?level=error" className="text-xs text-blue-600 hover:text-blue-800">
                View all
              </Link>
            </div>
                ...
                {dashboard.recent_errors.slice(0, 5).map((err, i) => (
                  <div key={i} className="text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-900 truncate">{err.message}</span>
                    </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-center gap-3">
              <span
                className={`w-3 h-3 rounded-full ${
                  tunnelStatus?.status === 'connected' ? 'bg-green-500' : 'bg-gray-300'
                }`}
              />
              <div>
                <div className="text-sm font-medium text-gray-900">Tunnel</div>
                <div className="text-xs text-gray-500">
