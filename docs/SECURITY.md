...
- Network access gated by admin approval with SSRF prevention (private IP blocking with optional port restriction, IP pinning, redirect restrictions)
...
- **Two-layer enforcement (defense-in-depth)**: Per-server domain enforcement at the application layer (SSRF client for HTTP, SafeSocket for raw TCP) + domain whitelisting at the SOCKS5 proxy layer. The proxy enforces a union of all servers' `allowed_hosts` for public destinations (plus infrastructure hosts like pypi.org, osv.dev, deps.dev). Even if the app-layer SSRF checks are bypassed, the proxy blocks connections to non-whitelisted hosts
