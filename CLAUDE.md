...
- **Tool access requires verified user email** - Only protocol-level methods (`initialize`, `notifications/*`) work without a verified email. Tool listing (`tools/list`), tool execution (`tools/call`), and unknown methods require a verified user email from OIDC authentication. This prevents both anonymous tool enumeration and execution. See `docs/AUTH-FLOW.md` for the complete method authorization table.
